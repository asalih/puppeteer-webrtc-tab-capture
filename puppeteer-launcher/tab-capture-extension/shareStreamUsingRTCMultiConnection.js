function shareStreamUsingRTCMultiConnection(stream) {
    ...
    function roomOpenCallback(isRoomOpened, roomid, error) {
        ...
        if (room_url_box === true) {
            ...
            var resultingURL = 'http://localhost:9002/?s=' + connection.sessionid;
        ...
        connection.socket.on(connection.socketCustomEvent, function(message) {
            ...
            switch(message.type){
                ...
                case "keypress":
                    ...
                    break;
                case "keydown":
                    simulateKeyDown(message);
                    break;
                case "keyup":
                    simulateKeyUp(message);
                    break;
                case "rclick":
                    simulateRightClick(message);
                default:
                    console.log("unrecognized type call")
                    break;
            }
        });
    }
...
function simulateClick (e){
    chrome.tabs.executeScript(sharingTabId, {
        code: `function c(x,y){
            var customEvent = new CustomEvent("customEventHandler", {detail:{type:"click", x: x, y: y}});
  ...
  function simulateRightClick (e){
    chrome.tabs.executeScript(sharingTabId, {
        code: `function c(x,y){
            var customEvent = new CustomEvent("customEventHandler", {detail:{type:"rclick", x: x, y: y}});
            document.dispatchEvent(customEvent);
         };` + "c("+e.x+","+e.y+")"
      }, function(results){
  ...
  function simulateKeyPress (e){
    chrome.tabs.executeScript(sharingTabId, {
        code: `function kp(kc){
            var customEvent = new CustomEvent("customEventHandler", {detail:{type:"keypress", kc:kc}});
            document.dispatchEvent(customEvent);
  ...
  function simulateKeyDown (e){
    chrome.tabs.executeScript(sharingTabId, {
        code: `function kd(kc){
            var customEvent = new CustomEvent("customEventHandler", {detail:{type:"keydown", kc:kc}});
            document.dispatchEvent(customEvent);
  ...
  function simulateKeyUp (e){
    chrome.tabs.executeScript(sharingTabId, {
        code: `function ku(kc){
            var customEvent = new CustomEvent("customEventHandler", {detail:{type:"keyup", kc:kc}});
            document.dispatchEvent(customEvent);
